...
const HistoricalIncomeStatement = ({ symbol }) => {
  ...
  const fetchHistoricalData = async () => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:8000/api/historical_income/${symbol}/`
      );
      ...
      setError(
        `Failed to fetch historical income data for ${symbol}: ${error.message}`
      );
    } finally {
      setLoading(false);
    }
  ...
  const downloadCSV = () => {
    ...
    if (link.download !== undefined) {
      ...
      link.setAttribute("download", `${symbol}_historical_income.csv`);
  ...
  return (
    <div className="historical-income-statement">
      <Title level={3}>{symbol} Historical Income Statement</Title>
      <Button
        type="primary"
...
export default HistoricalIncomeStatement;
